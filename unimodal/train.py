...
if __name__=="__main__":
    ...
    try:
        ...
        train_loader = create_loader(dataset_train, (3, 64, 64), 64, is_training=True)
    except:
        train_loader = create_loader(dataset_train, (3, 64, 64), 64, is_training=True, use_prefetcher=False)
    
    try:
        # only works if gpu present on machine
        val_loader = create_loader(dataset_val, (3, 64, 64), 64)
    except:
        val_loader = create_loader(dataset_val, (3, 64, 64), 64, use_prefetcher=False)
    
    model = timm.create_model('efficientnet_b0', pretrained=True, num_classes=10000, in_chans=3, checkpoint_path="").to(device)
    # model = model
